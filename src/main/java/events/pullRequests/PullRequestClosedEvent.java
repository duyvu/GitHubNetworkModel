 ...
 * 
 */
public class PullRequestClosedEvent extends Event {

	private static final Logger LOG = LoggerFactory
			.getLogger(PullRequestClosedEvent.class.getName());
	 ...
	 * @param time
	 */
	public PullRequestClosedEvent(double time, int pullRequestID, int userID,
			int repoID) {
		super(time);
	...
	public boolean equals(Object object) {
		PullRequestClosedEvent anotherEvent = (PullRequestClosedEvent) object;
	...
	public static int readEvents(String filePath, ArrayList<Event> events) {
		...
		try {
			...
			LOG.info("Reading pull request closed events from {}", filePath);
			...
			while ((line = reader.readLine()) != null) {
				...
				PullRequestClosedEvent pullRequestClosedEvent = new PullRequestClosedEvent(
						time, pullRequestID, userID, repoID);
				events.add(pullRequestClosedEvent);

				LOG.trace("Reading a pull request closed event[{}]",
						pullRequestClosedEvent);

			}
			reader.close();
		} catch (Exception e) {
			LOG.error("Errors in pull request closed events: " + filePath);
			e.printStackTrace();
			System.exit(-1);
		}
